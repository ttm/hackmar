...
U=r.URIRef
gg=r.Graph()
gg.parse("omarTriplestore.rdf")
print("Lido RDF")
def fazQuery(query):
    NOW=time.time()
    res=[i[0] for i in gg.query(query)]
    ...
    return res
g=r.Graph()
g.namespace_manager.bind("omar", "http://purl.org/socialparticipation/omar/")    
...
omar = r.Namespace("http://purl.org/socialparticipation/omar/")
xsd = r.namespace.XSD
#notFunctionalProperties=["tagged","contact","supporter"]
notFunctionalProperties=["hasReply","hasStep","knows","member","organization"]
notFunctionalProperties_=[omar+i for i in notFunctionalProperties]
...
PREFIX omar: <http://purl.org/socialparticipation/omar/>
...
#sparql.setReturnFormat(JSON)
#results = sparql.query().convert()
results=[i for i in fazQuery(PREFIX+q)]
...
for tkey in kk:
    ...
    A.graph_attr["label"]=("classe: %s, no namespace interno http://purl.org/socialparticipation/omar/"%(cl_,))
...
A=gv.AGraph(directed=True)
A.graph_attr["label"]="Diagrama geral da OPa para ao portal Participa.br no namespace interno: http://purl.org/socialparticipation/omar/"
ii=1
for tkey in kk:
    cl_=tkey.split("/")[-1]
...
for prop in props:
    ...
    P[prop_]=(suj,obj)
    A=gv.AGraph(directed=True)
    A.graph_attr["label"]=("propriedade: %s, no namespace interno: http://purl.org/socialparticipation/omar/"%(prop_,))
...
for prop in props:
    ...
    if len(ant)>1:
        ...
        for ant_ in ant:
            G(B, owl.unionOf, U(omar+ant_))
    elif ant:
        G(U(prop), rdfs.domain, U(omar+ant[0]))
        
    if len(cons)>1:
        B=r.BNode()
        G(U(prop), rdfs.range, B)
        for cons_ in cons:
            G(B, owl.unionOf, U(omar+cons_))
    elif cons:
        if "XMLS" in cons[0]:
            G(U(prop), rdfs.range, U(xsd+cons[0].split("#")[1]))
        else:
            G(U(prop), rdfs.range, U(omar+cons[0]))
...
A=gv.AGraph(directed=True)
A.graph_attr["label"]=r"Diagrama geral da OPa, parte dedicada aos dados do portal Participa.br, no namespace interno: http://purl.org/socialparticipation/omar/\nAresta em verde indica restricao existencial,\ncom a ponta invertida indica restricao universal,\ntracejada indica propriedade nao funcional"
ii=1
for tkey in kk:
    cl_=tkey.split("/")[-1]
    cl=tkey
...
for prop in props:
    ...
    suj,obj=P_[prop_]
    A=gv.AGraph(directed=True)
    A.graph_attr["label"]=(r"propriedade: %s, no namespace interno: http://purl.org/socialparticipation/omar/\nAresta em verde indica restricao existencial,\ncom a ponta invertida indica restricao universal,\ntracejada indica propriedade nao funcional"%(prop_,))
#    A.add_node(1,style="filled")
#    A.add_node(2,style="filled")
    A.add_edge(1,2)
    e=A.get_edge(1,2)
...
#g.namespace_manager.bind("xsd", r.namespace.XSD)    
#g.namespace_manager.bind("owl", r.namespace.OWL)    
#g.namespace_manager.bind("omar", "http://purl.org/socialparticipation/opa/")
#g.namespace_manager.bind("dc", "http://purl.org/dc/elements/1.1/")    
#g.namespace_manager.bind("dct", "http://purl.org/dc/terms/")    
#g.namespace_manager.bind("dcty", "http://purl.org/dc/dcmitype/")    
#g.namespace_manager.bind("gndo", "http://d-nb.info/standards/elementset/gnd#")    
...
f=open("OmarDados.owl","wb")
f.write(g.serialize())
f.close()
f=open("OmarDados.ttl","wb")
